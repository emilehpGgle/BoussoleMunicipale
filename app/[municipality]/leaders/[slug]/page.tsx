...
export default async function LeaderPage({ params }: LeaderPageProps) {
  ...
  const personJsonLd = {
    ...
    "image": leader.party.leaderPhotoUrl || leader.party.logoUrl,
    "knowsAbout": [
      ...
      <div className="container max-w-4xl py-12 px-4 md:px-6 space-y-8">
          ...
          <CardHeader className="bg-muted/30 p-6">
            <div className="flex flex-col sm:flex-row items-start gap-6">
              <div className="w-32 h-32 sm:w-40 sm:h-40 flex-shrink-0 relative bg-white rounded-xl p-3 shadow-sm">
                <Image
                  src={leader.party.leaderPhotoUrl || leader.party.logoUrl || "/placeholder.svg?width=160&height=160"}
                  alt={`Photo de ${leader.party.leader} - ${leader.party.name} - Élections municipales 2025`}
                  fill
                  style={{ objectFit: leader.party.leaderPhotoUrl ? "cover" : "contain" }}
                  className={leader.party.leaderPhotoUrl ? "rounded-lg" : ""}
                />
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Les priorités et objectifs de {leader.party.leader} pour la ville de {municipalityDisplay}
            </CardDescription>
          </CardHeader>
          <CardContent>
...
export async function generateMetadata({ params }: LeaderPageProps): Promise<Metadata> {
  ...
  return {
    title: `${party.leader} - Chef de ${party.name} | Profil Élections Municipales ${municipalityDisplay} 2025`,
    description: `Découvrez le profil complet de ${party.leader}, chef de ${party.name} pour les élections municipales de ${municipalityDisplay} 2025. Biographie, expérience, vision politique et réalisations.`,
    keywords: [
    ...
    openGraph: {
      title: `${party.leader} - Profil Politique | Élections Municipales ${municipalityDisplay} 2025`,
      description: `Profil complet de ${party.leader}, chef de ${party.name}. Découvrez son expérience, sa vision et ses positions pour les élections municipales ${municipalityDisplay} 2025.`,
      type: "profile",
      images: [
        {
          url: party.leaderPhotoUrl || party.logoUrl || "/hero-illustration-v2.webp",
          width: 1200,
          height: 630,
          alt: `${party.leader} - ${party.name} - Élections municipales ${municipalityDisplay} 2025`
        }
      ]
    },
    alternates: {
